...
export default function Sidebar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      logout(); // logout local de respaldo
    }
  };
  ...
  return (
    <div
      ...
      } ${resolvedTheme === "dark" ? "bg-blue-dark" : "bg-blue-primary"}`}
    >
      <div>
        ...
        <Link href={ROUTES.PROTECTED.PROFILE}>
          <div
            className={`flex items-center p-4 space-x-4 hover:bg-opacity-20 hover:bg-gray-700 rounded-md mx-2 ${
              pathname === ROUTES.PROTECTED.PROFILE
                ? resolvedTheme === "dark"
                  ? "bg-blue-violet/70"
                  : "bg-blue-medium/70"
                : ""
            ...
            <div className="flex items-center">
              ...
              {isExpanded && (
                <div className="ml-2">
                  ...
                  {isDoctor ? (
                    <span className="text-xs text-blue-sky">Doctor</span>
                  ) : (
                    <span className="text-xs text-gray-light">Paciente</span>
                  )}
                </div>
              )}
            </div>
              ...
              {NAVIGATION_ITEMS.doctor.map((item, index) => (
                <Link
                  ...
                  className={`flex items-center space-x-4 p-3 hover:bg-opacity-20 hover:bg-gray-700 rounded-md mx-2 ${
                    ...
                    isExpanded
                      ? resolvedTheme === "dark"
                        ? "bg-blue-dark/60 hover:bg-blue-violet/50"
                        : "bg-blue-primary/40 hover:bg-blue-medium/40"
                      : ""
                  }`}
                >
                  <span className="text-xl text-blue-sky">{getIcon(item.icon)}</span>
                  {isExpanded && (
                    <span className="text-sm font-medium text-blue-sky">{item.name}</span>
                  )}
                </Link>
              ))}
            </div>
